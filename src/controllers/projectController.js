...
const getTotalProjectsByService = async (req, res) => {
  try {
    ...
    // Si no se encuentran proyectos, devuelve un 404.
    if (projects.length === 0) {
      return res.status(404).json({ message: 'No se encontraron proyectos' });
    }
    ...
    console.error('Error en getTotalProjectsByService:', error);
    res.status(500).json({ message: 'Error del servidor' });
  }
};
...
const getInProgressProjectsCount = async (req, res) => {
  try {
    const count = await prisma.proyectos.count({ where: { estado: 'en_progreso' } });
...
const getProjectStatuses = async (req, res) => {
  try {
    // Use Prisma to get available project statuses from enum
    // Since Prisma doesn't directly support enum queries, we'll return the known values
    const estados = ['solicitado', 'en_progreso', 'finalizado', 'cancelado'];
